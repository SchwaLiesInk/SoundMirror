...
class GImage {
  ...
  void AddPixel(int x, int y,color c2) {
    int at=At(x, y);
    color c=bits.pixels[at];
    float r=red(c2)+red(c);
    float g=green(c2)+green(c);
    float b=blue(c2)+blue(c);
    float a=alpha(c2)+alpha(c);
    r*=0.5f;
    g*=0.5f;
    b*=0.5f;
    a*=0.5f;
